...
must_haves:
  ...
  key_links:
    ...
    - from: ".button:hover, .service-card:hover"
...
<task type="auto">
  ...
  <action>
Create or append to components.css on BOTH sites. If file doesn't exist, create it. Add mobile CTA bar styles at END of file.
...
If components.css doesn't exist yet, create it with standard file header:
...
**Important:** Use @media (max-width: 768px) to match existing breakpoint pattern. Use existing CSS variables (--color-accent, --space-md, --shadow-lg, etc.) for consistency.
  ...
  <verify>
```bash
# Check components.css exists and has mobile CTA
...
```
  ...
  <done>Both components.css files exist and contain .mobile-cta styles (fixed position, bottom: 0, orange accent background, z-index from variable) and body padding-bottom: 70px on mobile to prevent content overlap</done>
</task>
...
- `.button` class already exists on CTA buttons (verify with grep)
- `.service-card` class already exists on service cards (verify with grep)
  </action>
  <verify>
```bash
# Check hover effects exist in both sites
grep -n "button:hover" commercial/css/components.css
grep -n "service-card:hover" commercial/css/components.css
grep -n "button:hover" residential/css/components.css
...
# Check that .button and .service-card classes exist in HTML
grep -c "class=\"button" commercial/index.html
grep -c "class=\"service-card" commercial/index.html
```
  </verify>
  <done>Both components.css files contain button and service-card hover effects using translateY transform, @media (hover: hover) for desktop-only application, ::after pseudo-element for shadow optimization, and existing CSS variables for consistent styling</done>
</task>
...
**Also add components.css to <head> if not already there:**
...
3. **Test button hover (desktop)**
   ...
   - Hover over any .button element (CTA buttons)

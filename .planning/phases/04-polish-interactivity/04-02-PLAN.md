...
must_haves:
  ...
  artifacts:
    ...
    - path: "commercial/index.html"
      provides: "Mobile CTA HTML element and components.css link"
      contains: "mobile-cta"
...
<task type="auto">
  <name>Task 2: Create components.css with mobile CTA styling and link to HTML on both sites</name>
  <files>commercial/css/components.css, residential/css/components.css, commercial/index.html, residential/index.html</files>
  <action>
**Step 1: Create components.css on BOTH sites** (file does not currently exist)

Add mobile CTA bar styles:
...
**Step 2: Add components.css link to HTML on BOTH sites**

In the `<head>` section, after the site-specific CSS link (commercial.css or residential.css), add:

```html
  <link rel="stylesheet" href="css/components.css">
```

This ensures components.css is loaded immediately after creation.

Apply IDENTICAL CSS and HTML changes to both commercial and residential sites.
...
# CRITICAL: Check components.css is linked in HTML
grep "components.css" commercial/index.html
grep "components.css" residential/index.html

# Verify link placement (should be in <head> after site-specific CSS)
grep -B2 "components.css" commercial/index.html | grep -E "(commercial\.css|residential\.css)"
```
  </verify>
  <done>Both components.css files created with .mobile-cta styles (fixed position, bottom: 0, orange accent background, z-index from variable) and body padding-bottom: 70px on mobile. Both index.html files have <link rel="stylesheet" href="css/components.css"> in <head> after site-specific CSS, ensuring styles are loaded.</done>
</task>
...
**Note:** components.css link was added to <head> in Task 2, so CSS is already loaded at this point.
  </action>
  <verify>
```bash
...
# Confirm components.css was already linked in Task 2
grep "components.css" commercial/index.html
grep "components.css" residential/index.html
```
  </verify>
  <done>Both index.html files contain mobile CTA element with tel: link to 717-615-0968, aria-label for accessibility, positioned before closing </body> tag. Components.css was already linked in <head> during Task 2.</done>
</task>
...
1. **Test mobile CTA (mobile viewport)**
   ...
   - Scroll up → button stays at bottom (doesn't scroll away)
   - Footer content should NOT be hidden behind button (70px body padding)
   - Tap button → should trigger phone call (tel: link)
...
3. **Test button hover (desktop)**
   ...
   - Move mouse away → button returns to normal
   - Click button → slight press effect (translateY -2px)
...
4. **Test service card hover (desktop)**
   ...
   - Move mouse away → card returns to normal
   - Repeat on both sites
...
<success_criteria>
**Mobile CTA working:**
- User opens commercial site on mobile (375px) → orange "Call Now: 717-615-0968" button fixed at bottom of screen
- User scrolls page → button stays at bottom (fixed position)
- User scrolls to footer → footer content visible, not covered by button (body padding working)
- User taps button → phone dialer opens with 717-615-0968 pre-filled
- User opens on desktop (1024px+) → mobile CTA not visible
- Residential site has identical mobile CTA behavior

**Desktop hover effects working:**
- User hovers over CTA button on desktop → button lifts up 4px, shadow increases (smooth transition)
- User moves mouse away → button returns to original position
- User hovers over service card → card lifts up 8px, shadow increases
- All hover transitions are smooth (60fps, no jank)

**Mobile hover prevented:**
- User taps button on mobile → no lift effect, no sticky hover state
- User taps service card on mobile → no lift effect
